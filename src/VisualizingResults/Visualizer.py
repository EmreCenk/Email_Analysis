...
from src.EmailProcedures.ParsingAndSavingEmails import count_all_emails, count_words
import matplotlib.pyplot as plt
from src.EmailProcedures.utils import group_uni_emails, sort_dictionary_on_values
...
class Visualizer():
    ...
    def visualize_university_email_numbers(group_emails: bool = True):
        ...
        if group_emails:
            email_count, grouped = group_uni_emails(email_count)
    ...
    @staticmethod
    def visualize_university_word_count():
        email_count = count_words()
        sorted = sort_dictionary_on_values(email_count)
        uni_names = []
        values = []
        for name in sorted:
        ...
        plt.title("Most Common Words Used in Uni Emails")
        plt.show()
